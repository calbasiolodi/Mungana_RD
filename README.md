...
XGBoost was trained on a dataset containing a small numbers of Minalyzer measurements (n=15) that contain the type of mienralisation to be identified on other drill cores (883, 845 and RD 197), in addition random datapoints with Cu_pct>=1 from the drillocore RD997 to bring to a total of n=50, have been added in the training dataset to avoid single class training and data leakage (i.e., adding random entries from the test drill cores). The practical goal is not achieving the recongnition of each datapoint in the other drillcores, rather it is assessing whether it is present in a specific mineralised zone and at which depths. This is for practicality, since this meter scale resolution is sufficent since extraction would occur on the meter-scale not on a decimetric one. In addition, mineralised zones might contain two types of mineralisation in a sub-metric interval, rendering mixing of different mineralisation inevitable when the rock is loaded into excavators. The purpose is then idenifying the depth and whether a mineralised zone contains the "ambiguous" mineralisation type.
